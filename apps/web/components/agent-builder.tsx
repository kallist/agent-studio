...
export function AgentBuilder({ saving, providerReadiness, knowledgeBases, knowledgeLoading, knowledgeError, onReloadKnowledge, onSave }: AgentBuilderProps) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (draft.runtime !== "mock" && !draft.model.trim()) { setValidation(`Enter a provider model ID for ${draft.runtime === "openai" ? "OpenAI" : "DeepSeek"} mode.`); return; }
    setValidation(null);
    ...
    <>
      ...
      <div className="builder-layout">
        <form className="surface builder-form" onSubmit={submit}>
          ...
          <label className="field"><span>Model</span><small>{draft.runtime === "mock" ? "The mock runtime is deterministic and does not call a model." : selectedProvider?.configured ? `${draft.runtime === "openai" ? "OpenAI" : "DeepSeek"} key configured. Use the exact model ID to run.` : `${draft.runtime === "openai" ? "OpenAI" : "DeepSeek"} key not configured. Definitions can be saved, but runs will be blocked.`}</small><input value={draft.runtime === "mock" ? "Deterministic mock" : draft.model} onChange={(event) => set("model", event.target.value)} placeholder="Enter provider model ID" disabled={saving || draft.runtime === "mock"} required={draft.runtime !== "mock"} /></label>
