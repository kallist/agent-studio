        ...
        <section className="agent-card-grid" aria-label="Agent definitions">
          {agents.map((agent) => {
            ...
            return <article key={agent.id} className={agent.id === selectedId ? "agent-definition-card selected" : "agent-definition-card"}>
              ...
              <dl className="agent-metadata"><div><dt>Tools</dt><dd>{agent.tools.length ? agent.tools.map((tool) => <span className="tool-chip" key={tool}><Icon name={tool === "calculator" ? "calculator" : tool === "knowledge_search" ? "knowledge" : "tool"} />{tool}</span>) : <span className="muted-value">None</span>}</dd></div><div><dt>Knowledge base</dt><dd>{boundKnowledge.length ? boundKnowledge.map((name) => <span className="tool-chip" key={name}><Icon name="knowledge" />{name}</span>) : <span className="muted-value"><Icon name="knowledge" />Not bound</span>}</dd></div><div><dt>Last run</dt><dd>{lastRun ? <><StatusBadge status={lastRun.status} /><span className="meta-time">{formatRelativeTime(lastRun.updated_at)}</span></> : <span className="muted-value">Never</span>}</dd></div></dl>
